...
export default class MathField {
  ...
  _createMathfield() {
    ...
    mfe.addEventListener('input', (ev) => {
      ...
      // Вызываем колбэк onChange, если он был предоставлен
      if (typeof this.config.onChange === 'function') {
        this.config.onChange(ev.target.value);
      }
    });
    ...
    mfe.addEventListener('keydown', (ev) => {
      if (ev.key === 'Delete') {
        ev.preventDefault();
        // Удаление последнего символа из LaTeX-строки
        this.data.latex = this.data.latex.slice(0, -1);
        // Обновляем значение в MathfieldElement
        mfe.setValue(this.data.latex);
        // Вызываем колбэк onChange, если он был предоставлен
        if (typeof this.config.onChange === 'function') {
          this.config.onChange(this.data.latex);
        }
      }
    });
